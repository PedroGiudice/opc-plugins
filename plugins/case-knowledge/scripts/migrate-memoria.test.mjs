...
// project_* que casam limpo (exatamente 1 pasta plausivel). Nomes verbatim.
const CLEAN = [
  ...
  ["project_abrafarma_ml_ifood.md", "abrafarma-ML-Ifood"],
  ["project_salesforce_facilita_attrus.md", "salesforce-facilita-pagamentos"],
];

test("classifyLegacyMemFile: project_* limpos casam na pasta certa (kind project + casePath)", () => {
  for (const [name, expected] of CLEAN) {
...
test("classifyLegacyMemFile: bianka casa (caso nomeado do brief)", () => {
...
test("classifyLegacyMemFile: odmgt_contratos_modelo e ORFAO (nao casa com odmgt-jf-beleza)", () => {
  const r = classifyLegacyMemFile("project_odmgt_contratos_modelo.md", CASE_PATHS);
  assert.equal(r.orphan, true);
  assert.equal(r.casePath, undefined);
});

test("classifyLegacyMemFile: as 3 compassion casam na unica pasta compassion", () => {
  for (const name of [
...
test("invariante de seguranca: ambiguo/orfao NUNCA produzem casePath", () => {
  const ALL = [
...
test("planMigration: mapeia todos os nomes e conta por categoria", () => {
  const names = [
    "project_bianka_salesforce.md", // clean
...
test("migrationDest: project com casePath -> <base>/<caso>/.memoria/<autor>/<nome>", () => {
  const entry = { name: "project_bianka_salesforce.md", kind: "project", casePath: "bianka-salesforce" };
...
test("migrationDest: feedback -> <base>/.feedback/<autor>/<nome>", () => {
